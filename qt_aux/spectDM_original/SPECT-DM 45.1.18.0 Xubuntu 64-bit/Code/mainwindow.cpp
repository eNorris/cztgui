...
void MainWindow::OperationErrorCallbackFunc(OperationType a_Type,
    const char* a_Data
)
{
    switch(a_Type)
    {
    case OperationType_SavePackets:
...
void MainWindow::OperationCompleteCallbackFunc(OperationType a_Type)
{
    switch(a_Type)
    {
    case OperationType_SavePackets:
        GetInstance()->CloseProgressDlg();
        break;
    case OperationType_Collection:
        GetInstance()->OnCollectionComplete();
        break;
    }
}

void MainWindow::OperationProgressCallbackFunc(OperationType a_Type,
    int a_Progress
)
{
    switch(a_Type)
...
void MainWindow::SetupSpectUI()
{
    ...
    // Fill in the collection time combo
    const int maxTime = 15 * 60; // 15 minutes
    const int timeIncrement = 30; // Increment in seconds per entry in the combo
    ui->collectionTimeCombo->addItem(QString("No limit"), QVariant(0));
    for (int time = timeIncrement; time < maxTime; time +=timeIncrement)
    {
        int mins = time / 60;
        int seconds = time % 60;
        ui->collectionTimeCombo->addItem(QString("%1 minutes %2 seconds").arg(mins).arg(seconds), QVariant(time));
    }
    ...
    ui->collectionTimeCombo->setEnabled(false);
...
{
    int timeout = ui->collectionTimeCombo->currentData().toInt(0);
    if(SpectDMDll::StartPhotonCollection(timeout))
    {
        ui->stopSysBtn->setEnabled(false);
        ui->startCollectBtn->setEnabled(false);
        ui->collectionTimeCombo->setEnabled(false);
...
void MainWindow::OnCollectionComplete()
{
    ui->stopSysBtn->setEnabled(true);
    ui->stopCollectBtn->setEnabled(false);
    ui->startCollectBtn->setEnabled(true);
    ui->collectionTimeCombo->setEnabled(true);
}

void MainWindow::on_stopCollectBtn_clicked()
{
    // Call stop collection method, OnCollectionComplete callback will be triggered once collection is actually finished and the
    // ui can be updated
